...
export function PlayerApp({ preview, context }: { preview: boolean; context?: TapFederatedSurfaceMountContext }) {
  ...
  const togglePlay = useCallback(async (): Promise<void> => {
    ...
    if (!url) {
      reportError(new Error(preview ? "The browser preview audio record is missing. Import the owned file again." : "TAP SDK 0.7.0 cannot read packaged binary artifacts after a page reload. Re-import this owned file to restore session playback."), "Audio is unavailable.");
      return;
    }
    if (!audio.src) {
      audio.src = url;
