...
import { canViewSunoPlayer } from "./authorization";
import {
  actorFeedback,
...
export function PlayerApp({ preview, context }: { preview: boolean; context?: TapFederatedSurfaceMountContext }) {
  ...
  const activeWorkspaceId = preview
    ? context?.workspaceId
    : (owner?.workspaceId ?? context?.workspaceId);
  const activeOwnerChannelId = preview ? null : owner?.channelId;
  ...
  const [channelId, setChannelId] = useState(() =>
    initialPlayerChannelId(preview, activeOwnerChannelId ?? context?.channelId),
  );
  ...
  const [providerCapabilities, setProviderCapabilities] = useState<ProviderCapabilityReport | null>(null);
  const [providerCapabilityError, setProviderCapabilityError] = useState("");
  ...
  useEffect(() => {
    ...
    const update = (): void => {
      const nextAuthority = context.hostAuthority.getSnapshot();
      setAuthority(nextAuthority);
      if (!nextAuthority) setViewAllowed(null);
    };
    update();
  ...
  useEffect(() => {
    ...
    const authorize = async (): Promise<void> => {
      const allowed = await canViewSunoPlayer();
      if (!active) return;
      setViewAllowed(allowed);
      if (!allowed) {
        ...
        setStatus("error");
        setError(
          "TAP did not grant suno-player.view. The player will not read channels, identity, or stored soundtrack data.",
        );
      }
    };
    void authorize();
  ...
  useEffect(() => {
    ...
    if (preview) {
      setProviderCapabilities({ hostHttp: false, credentialDiscovery: false, credentials: [] });
      setProviderCapabilityError("");
      return;
    }
    if (!authority || viewAllowed !== true) return;
    const inspect = async (): Promise<void> => {
      try {
        const report = await inspectProviderCapabilities(sdk);
        if (active) {
          setProviderCapabilities(report);
          setProviderCapabilityError("");
        }
      } catch (caught) {
        if (active) setProviderCapabilityError(errorMessage(caught, "Host credential metadata could not be listed."));
      }
    };
    void inspect();
    return () => {
  ...
  }, [authority, preview, tab, viewAllowed]);
  ...
  useEffect(() => {
    ...
    const loadChannels = async (): Promise<void> => {
      try {
        const result = await sdk.channels.list(activeWorkspaceId ? { workspaceId: activeWorkspaceId } : {});
  ...
  }, [activeWorkspaceId, authority, preview, reportError, viewAllowed]);

  useEffect(() => {
    let active = true;
    if (preview) return;
    if (!authority || viewAllowed !== true || !context) return;
  ...
  }, [authority, context, preview, viewAllowed]);
  ...
  const reload = useCallback(async (): Promise<void> => {
    ...
    try {
      if (!preview) {
        const access = await sdk.channels.getAccess(activeWorkspaceId ? { workspaceId: activeWorkspaceId, channelId } : { channelId });
  ...
  }, [activeWorkspaceId, authority, channelId, installChannel, installPreferences, preview, reportError, storagePort, userKey, viewAllowed]);
  ...
  const togglePlay = useCallback(async (): Promise<void> => {
    ...
    if (!url) {
      reportError(new Error(preview ? "The browser preview audio record is missing. Import the owned file again." : "This player has not migrated imported audio to TAP SDK 0.9.0 profile files yet. Re-import this owned file to restore session playback."), "Audio is unavailable.");
      return;
    }
    if (!audio.src) {
      audio.src = url;
  ...
  }, [authority, channelId, preferences?.broadcastPresence, preview, reportError, viewAllowed]);
  ...
  }, [authority, channelId, currentTrack?.id, playing, preferences?.broadcastPresence, preview, reportError, viewAllowed]);
  ...
  useEffect(() => {
    if (activeOwnerChannelId && activeOwnerChannelId !== channelId) {
      void switchChannel(activeOwnerChannelId);
    }
  }, [activeOwnerChannelId, channelId, switchChannel]);
  ...
  useEffect(() => {
    ...
    const loadCapabilities = async (): Promise<void> => {
      ...
      try {
        ...
        if (api) {
          const result = await api.listWorkspace(activeWorkspaceId);
      ...
      try {
        const result = await sdk.workflows.list({ workspaceId: activeWorkspaceId });
  ...
  }, [activeWorkspaceId, authority, preview, tab, viewAllowed]);
  ...
  const loadTimeline = async (): Promise<void> => {
    ...
    try {
      ...
      const result = await sdk.channels.getTimeline(activeWorkspaceId ? { workspaceId: activeWorkspaceId, channelId } : { channelId });
  ...
  if (startupView === "confirming-access") {
    return <main className="shell centered" aria-busy="true">
      ...
      <p className="muted">TAP is checking the exact surface view permission before any protected data is read.</p>
      <Progress value={35} aria-label="Confirming player access" />
    </main>;
  }
