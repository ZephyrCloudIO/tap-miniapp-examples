...
- [x] SDK dependency and `compatibility.tapSdk` are exact-pinned to `0.7.0`.
...
- [ ] `BLOCKED` Platform-wide message/thread ineligibility and retention/deletion requests: SDK `0.7.0` exposes timeline reads but no channel-message eligibility metadata, artifact deletion, or retention-request contract. A local flag would not govern other TAP readers.
...
- [ ] `BLOCKED` Creating recurring schedules: SDK `0.7.0` supports package-declared workflows but exposes no schedule creation or recurring-trigger API.
...
- [ ] `BLOCKED` Reloadable packaged binary audio/artwork artifacts: SDK `0.7.0` storage is JSON-only and VFS is write-only; no readable channel-artifact API exists.
- [ ] `BLOCKED` Zephyr Cloud publication from the player runtime: SDK `0.7.0` supports package publication through a configured build-time adapter, but exposes no miniapp runtime publication or receipt contract.
...
- [x] Retained-surface channel following uses the live SDK `0.7.0` owner snapshot and pauses/reloads safely when TAP selects a different channel.
- [ ] `BLOCKED` Host-wide playback continuity and dock/side/popup/minimized representations: SDK `0.7.0` has no global playback-session, docking, popup, or canonical shared-queue API.
...
- [ ] `BLOCKED` Canonical synchronized channel-session queue/position/control authority: presence is ephemeral and SDK `0.7.0` exposes no durable shared-session primitive.
