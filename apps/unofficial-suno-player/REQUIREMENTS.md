...
- [x] SDK dependency and `compatibility.tapSdk` are exact-pinned to `0.4.1`.
...
- [ ] `BLOCKED` Platform-wide message/thread ineligibility and retention/deletion requests: SDK `0.4.1` exposes timeline reads but no channel-message eligibility metadata, artifact deletion, or retention-request contract. A local flag would not govern other TAP readers.
...
- [ ] `BLOCKED` Creating recurring schedules: SDK `0.4.1` supports package-declared workflows but exposes no schedule creation or recurring-trigger API.
...
- [ ] `BLOCKED` Reloadable packaged binary audio/artwork artifacts: SDK `0.4.1` storage is JSON-only and VFS is write-only; no readable channel-artifact API exists.
- [ ] `BLOCKED` Zephyr Cloud publication: Marketplace categories are declared, but SDK `0.4.1` exposes no publication API or receipt contract.
...
- [ ] `BLOCKED` Host-wide playback continuity, dock/side/popup/minimized representations, and automatic current-channel following: SDK `0.4.1` has no global playback-session, docking, popup, or current-channel subscription API.
...
- [ ] `BLOCKED` Canonical synchronized channel-session queue/position/control authority: presence is ephemeral and SDK `0.4.1` exposes no durable shared-session primitive.
