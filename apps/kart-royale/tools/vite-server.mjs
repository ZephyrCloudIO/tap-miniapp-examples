 ...
 * orphan keeps port 5173 bound, and the next dev server — or the user's own
 * `pnpm dev` — dies with "Port 5173 is already in use" long after the harness
 * that started it has exited.
 *
 * The game now builds with rsbuild (the TAP miniapp toolchain is rspack-based),
 * so the spawned binary is `rsbuild dev`; nothing else about the lifecycle
 * changed. The filename stays `vite-server.mjs` because every harness imports
 * it by that path.
 *
 * Two fixes, both needed:
 *   1. spawn the dev-server binary directly, so the handle IS the server;
...
export async function startVite(port, { timeoutMs = 36000 } = {}) {
  ...
  const bin = join(root, 'node_modules/.bin/rsbuild');
  if (!existsSync(bin)) throw new Error(`rsbuild binary not found at ${bin} — run pnpm install`);

  const proc = spawn(bin, ['dev', '--port', String(port)], {
    cwd: root,
    stdio: 'ignore',
    // Own process group, so stop() can take down anything rsbuild itself spawned.
    detached: true,
  });
  ...
  stop();
  throw new Error(`rsbuild did not come up on port ${port} within ${timeoutMs}ms`);
}
