...
export class HUD implements System {
  ...
  init(ctx: Ctx) {
    // `uiHost` is the `#ui` element inside the game host, published by main —
    // the same pattern as `(ctx as any).materials`. The document-global
    // fallback keeps older harnesses working; the element lookup is last
    // because a packaged TAP mount must not resolve ids outside its surface.
    const host =
      (ctx as any).uiHost ?? document.getElementById('ui') ?? document.body;
