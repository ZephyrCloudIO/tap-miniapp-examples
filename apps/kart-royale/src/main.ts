...
/**
 * Handle returned by {@link startKartRoyale}. The game was born as a full-page
 * module-side-effect app; the TAP miniapp needs it to be a mountable,
 * disposable component, so everything below lives inside one function scope
 * and every global it touched is unwound by `dispose()`.
 */
export interface KartRoyaleHandle {
  /** The live context, exposed for harnesses (`__ctx` parity). */
  readonly ctx: Ctx;
  /**
   * Stop simulating and presenting while keeping the world alive — the TAP
   * lifecycle's pause/resume and host-hidden states drive this. Equivalent to
   * the loop's own `suspended` flag, plus the audio context.
   */
  setSuspended(suspended: boolean): void;
  /** True once dispose() has run. */
  readonly disposed: boolean;
  /** Cancel the loop, remove every listener, dispose the renderer and the DOM. */
  dispose(): void;
}

/**
 * Boot the game into `host`, an element prepared by `createGameHost()` (it must
 * contain `#app`, `#ui` and `#boot` children — see `hostDom.ts`). The host can
 * be a full-viewport page element (standalone preview) or a TAP surface
 * container (packaged miniapp); the game only ever queries inside it.
 */
export function startKartRoyale(host: HTMLElement): KartRoyaleHandle {
const parent = host.querySelector<HTMLElement>('#app') ?? host;
const uiHost = host.querySelector<HTMLElement>('#ui') ?? parent;
...
// The HUD overlay host (`#ui` inside the game host). HUD reads this instead of
// a document-global id lookup so a packaged mount inside a TAP surface cannot
// collide with — or leak into — the surrounding document.
(ctx as any).uiHost = uiHost;
...
function bootProgress(frac: number, label: string) {
  const bar = host.querySelector<HTMLElement>('.boot-bar i');
  const step = host.querySelector<HTMLElement>('.boot-step');
...
function dismissBootScreen() {
  const boot = host.querySelector<HTMLElement>('#boot');
  if (!boot) return;
...
let suspended = false;
/** Set by dispose(); the frame loop checks it before re-queueing itself. */
let disposed = false;
/** The optional screen recorder, held so dispose() can detach its listener. */
...
function uninstallResizeListeners() {
  removeEventListener('resize', queueResize);
  removeEventListener('orientationchange', queueResize);
...
const DEBUG_GLOBALS = ['__ctx', '__drawBudget', '__camRig', '__loopHealth', '__gameReady', '__feel'] as const;
...
return {
  ...
  setSuspended(next: boolean) {
    if (disposed) return;
    suspended = next;
    // Mirrors what the Audio system's own visibilitychange handler does; a TAP
    // surface can be paused without the document ever becoming hidden.
    (audio as unknown as { setSuspended?: (v: boolean) => void }).setSuspended?.(next);
    if (!next) {
      // Same fresh-clock handoff as the context-restore path: the first frame
      // back must not span however long the surface was paused.
      last = performance.now();
      lastTickPresented = false;
    }
  },
  dispose() {
    ...
    input.dispose();
    pipeline.dispose();
    for (const name of DEBUG_GLOBALS) delete (window as any)[name];
    // The host element itself belongs to the caller; its children are ours.
