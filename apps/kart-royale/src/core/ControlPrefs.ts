...
/**
 * Raw-record storage behind `load()`/`save()`. The default is the game's
 * original wrapped localStorage, still used by the standalone browser preview.
 * A packaged TAP surface installs a host-storage backend instead — hydrated
 * before the game boots, so `load()` stays synchronous and the rule "nothing
 * in the boot path awaits this" survives the swap.
 */
export interface ControlPrefsBackend {
  /** The raw stored record, or null. Must never throw. */
  read(): string | null;
  /** Best-effort write; must never throw. */
  write(raw: string): void;
}

const localStorageBackend: ControlPrefsBackend = {
  read() {
    try {
...
let backend: ControlPrefsBackend = localStorageBackend;

/** Swap the raw store. Install before the game systems initialise. */
export function installControlPrefsBackend(next: ControlPrefsBackend): void {
  backend = next;
}
...
export function save(p: ControlPrefsData): void {
  backend.write(JSON.stringify(p));
}
