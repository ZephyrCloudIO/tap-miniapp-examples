...
/**
 * The multiplayer authority hand-off. `Items` calls these for karts this
 * client simulates; the NetAdapter implements them as server requests.
 */
export interface ItemsNetDriver {
  /** A local kart touched box `boxIndex`. True = the server owns the draw. */
 ...
 */
const BOX_RESPAWN = BOX_RESPAWN_S;
...
const KINDS = ITEM_KINDS as ItemKind[];
...
export class Items implements IItems {
  ...
  /**
   * Multiplayer authority hand-off. When attached, box pickups and item spends
   * for locally-simulated karts are REQUESTS to the session server; the box
   * state and the granted item land back on the broadcast channel. Solo play
   * leaves this null and keeps the local roll.
   */
  netDriver: ItemsNetDriver | null = null;
  private ctx!: Ctx;
  ...
  roll(place: number, racers: number): ItemKind {
    // The shared table, with local entropy — the session server rolls from
    // the same curve with its seeded generator in multiplayer.
    return rollItem(Math.random, place, racers) as ItemKind;
  }
  ...
  pickup(kart: IKart, boxIndex = -1) {
    ...
    if (this.netDriver?.requestDraw(kart, boxIndex)) {
      // The roulette spins while the server rolls from the shared table; the
      // grant lands in grantItem(). The pickup event (and its audio cue) fire
      // on contact, exactly as they do for a local roll.
  ...
  /**
   * Complete a server-granted draw for one of MY karts. Identity with a local
   * roll's effect, minus the roll: the room's seeded generator decided `kind`.
   */
  grantItem(kart: IKart, kind: ItemKind, count = 1) {
    const s = this.slot(kart);
    if (s.kind !== ItemKind.None || s.carried >= 0) return;
    this.give(kart, kind, count);
  }

  /** Server says a box is down/up (the authoritative course state). */
  setBoxDown(boxIndex: number, down: boolean, respawnInS = BOX_RESPAWN) {
    ...
    if (down) {
      ...
      b.down = 0;
      if (b.scale <= 0.005) b.scale = 0.001; // replay the elastic pop-in
    }
  }
  ...
  use(kart: IKart, backwards: boolean): boolean {
    ...
    // Multiplayer: a spend is a server round trip. The roulette stays honest
    // because the room owns the inventory; the effect lands in confirmUse().
    if (this.netDriver) return this.netDriver.requestUse(kart, backwards);
    ...
    const kind = s.kind;
    if (!this.applyUseEffect(kart, kind, backwards, s)) return false;
  ...
  /**
   * The effect half of `use()`, split out so the multiplayer confirm path runs
   * the identical effect after the server's inventory check. Returns false
   * when nothing was spent (unknown kind, or the projectile pool is full).
   */
  private applyUseEffect(kart: IKart, kind: ItemKind, backwards: boolean, s: Slot): boolean {
    switch (kind) {
  ...
  /**
   * Server-validated spend for one of MY karts: run the effect and mirror the
   * decrement. The room owns the inventory; a stale/echo broadcast no-ops.
   */
  confirmUse(kart: IKart, kind: ItemKind, backwards: boolean): void {
    const s = this.slot(kart);
    // Releasing a towed shield spends the grant without spawning a second item.
    if (s.carried >= 0) {
      if (this.proj.isCarried(s.carried, kart.id)) {
        const target = backwards ? -1 : this.targetAhead(kart);
        this.proj.release(s.carried, kart, backwards, target);
    ...
    if (s.kind !== kind || s.count <= 0) return;
    if (!this.applyUseEffect(kart, kind, backwards, s)) return;
    this.ctx.bus.emit({ type: 'item-use', kart, kind });
    s.count--;
    if (s.count <= 0) {
      s.kind = ItemKind.None;
  ...
  /**
   * A remote owner's spend: projectiles render as remote-visuals (the shooter's
   * client owns hit claims); self-effects (mushroom/star) and the bolt are
   * applied where the affected karts live, never here.
   */
  confirmUseRemote(kart: IKart, kind: ItemKind, backwards: boolean, carry: boolean, target: number): void {
    switch (kind) {
      case ItemKind.GreenShell:
      case ItemKind.RedShell:
      case ItemKind.Banana:
  ...
  /**
   * The bolt's per-victim effect, factored out of fireBolt so the multiplayer
   * adapter can apply it to exactly the karts this client owns (everyone else
   * applies it to theirs).
   */
  boltHit(k: IKart) {
    if (k.starTime > 0) return;
    const before = k.stunTime;
    k.squash(BOLT_STUN);
    if (k.stunTime <= before) return;   // invulnerable, nothing landed
    const s = this.slot(k);
    s.shrink = BOLT_TIME;
  ...
  /** Set by the multiplayer adapter: projectile contact vs network-owned karts. */
  set remoteHitHandler(fn: ((kart: IKart, kind: ItemKind) => void) | null) {
    this.proj.onRemoteHit = fn;
  }
    ...
    for (let i = 0; i < this.boxes.length; i++) {
      ...
      if (b.down > 0) {
        ...
        for (let j = 0; j < karts.length; j++) {
          ...
          // Network-owned karts collect boxes on their owner's client; taking
          // one here would both rob them and double-grant the item.
          if (isRemoteKart(k)) continue;
