...
interface Proj {
  ...
  targetId: number;
  homing: boolean;
  /** remote owner's visual: animates and expires, but never resolves hits here */
  remote: boolean;
  mesh: THREE.Mesh;
}
...
export class Projectiles {
  ...
  init(ctx: Ctx) {
    ...
    for (let i = 0; i < POOL; i++) {
      ...
      this.pool.push({
        ...
        targetId: -1, homing: false, remote: false, mesh,
      });
    }
  ...
  /**
   * A remote owner's projectile, rendered from the same launch math but
   * flagged visual-only: it animates and expires here while hit arbitration
   * happens on the shooter's client (claim → relay → victim applies).
   */
  spawnRemote(kind: ItemKind, owner: IKart, backwards: boolean, carried: boolean, targetId = -1): number {
    const idx = this.spawn(kind, owner, backwards, carried, targetId);
    if (idx >= 0) this.pool[idx].remote = true;
    return idx;
  }
   ...
   */
  onRemoteHit: ((kart: IKart, kind: ItemKind) => void) | null = null;

  /** Release a remote owner's towed item, visually. Returns false if none. */
  releaseRemote(ownerKart: IKart, backwards: boolean, targetId = -1): boolean {
    for (const p of this.pool) {
      if (p.remote && p.state === PState.Carried && p.owner === ownerKart.id) {
        const kind = p.kind;
        p.state = PState.Free;
        p.mesh.visible = false;
        return this.spawnRemote(kind, ownerKart, backwards, false, targetId) >= 0;
      }
    }
    return false;
  }
  ...
  private testKarts(ctx: Ctx, p: Proj, karts: readonly IKart[]): boolean {
    // A remote owner's visual never resolves hits on this client.
    ...
    for (let i = 0; i < karts.length; i++) {
      ...
      if (isRemoteKart(k)) {
        // The victim's client owns the effect; we own the detection.
        this.onRemoteHit?.(k, p.kind);
        if (p.kind === ItemKind.Bomb) this.explode(ctx, p, karts);
        this.kill(p);
        return false;
      }
  ...
  private explode(ctx: Ctx, p: Proj, karts: readonly IKart[], scale = 1) {
    ...
    for (let i = 0; i < karts.length; i++) {
      ...
      _v.subVectors(k.position, p.pos);
      const blastD = _v.length();
      if (blastD > r || Math.abs(_v.y) > 5) continue;
      if (isRemoteKart(k)) {
        // A blast victim we do not own is claimed, not struck.
        if (!p.remote) this.onRemoteHit?.(k, ItemKind.Bomb);
        continue;
      }
      if (p.remote) continue; // a remote owner's blast is visual-only here
