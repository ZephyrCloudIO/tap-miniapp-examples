...
import type { RaceNetHooks } from '../net/netHooks';
...
export class Race implements IRace {
   ...
   * is run inside a room. `null` in solo play: every kart is local then.
   */
  net: RaceNetHooks | null = null;

  /**
   * Slot-assigned grid positions in a networked race. Solo play puts the
   * player on pole (`slotFor`); in a room the server owns slot assignment and
   * the grid maps one-to-one, so no kart trades places with anyone.
   */
  private slotFor(i: number): number {
    if (this.net) return i;
   ...
   * `mySlot` null means this client is spectating — every kart is remote and
   * the camera anchors on pole. Call before `beginNetworkRace`.
   */
  setNetworkRoster(
    ...
    if (mySlot !== null && this.karts[mySlot]) {
      ...
      this.karts[mySlot].isPlayer = true;
      this.selected = mySlot;
      this.player = this.karts[mySlot];
    } else if (mySlot === null) {
  ...
  /**
   * Server-driven start: arm the countdown and sync it to the room's clock so
   * every client drops the lights at the same wall-clock instant.
   */
  beginNetworkRace(endsAtMs: number, serverNowMs: number) {
    this.start();
    this.countdownT = Math.max(0, (endsAtMs - serverNowMs) / 1000);
  }

  /** A remote kart's finish, published by the server. */
  markRemoteFinished(kartId: number, place: number) {
    const k = this.karts[kartId];
    const p = this.prog[kartId];
    if (!k || !p || k.finished) return;
    k.finished = true;
    p.finishOrder = place;
  ...
  update(ctx: Ctx, dt: number) {
    ...
    for (let i = 0; i < this.karts.length; i++) {
      ...
      // Networked karts pose from the session stream, not from local physics.
      // Their checkpoints, respawns and finishes are arbitrated by their owner
      // (and the server for human players), so none of the local bookkeeping
      // below applies to them.
      if (this.net?.isRemote(k)) {
        this.net.applyRemote(ctx, k, dt);
        continue;
      }
    ...
    // Uplink after progress so the session sees the freshest raceDistance.
    // Spectators have no kart of their own (isPlayer is false everywhere), so
    // the camera anchor must not be streamed as if it were ours.
    if (this.net && this.player?.isPlayer) {
      const p = this.prog[this.player.id];
      this.net.onLocalFrame(ctx, this.player, this.player.lap, p?.cp ?? 0, this.player.raceDistance);
    }
  }
  ...
  private updateProgress() {
    ...
    for (let i = 0; i < this.karts.length; i++) {
      const k = this.karts[i];
      const p = this.prog[i];
      // Remote karts keep their owner's reported progress; validating it here
      // as well would double-count crossings the owner already arbitrated.
      if (this.net?.isRemote(k)) continue;
      ...
      if (fwd === 1) {
        ...
        if (this.net && k.isPlayer) {
          this.net.onCheckpointClaim(p.lapIndex, c, k.raceDistance);
        }
      } else if (fwd === N - 1) {
  ...
  private crossLine(k: Kart, p: Progress) {
    ...
    if (p.lapIndex >= this.totalLaps && !k.finished) {
      ...
      if (this.net && k.isPlayer) this.net.onFinish(this.raceTime);
    }
  }
