...
export class Audio implements System {
  ...
  private syncSuspend() {
    ...
    if (!s) return;
    try {
      if (document.hidden || this.hostSuspended) s.ctx.suspend().catch(() => {});
  ...
  /**
   * True while the host surface (TAP lifecycle) has the game paused. Document
   * visibility is not the only suspension source any more: a packaged miniapp
   * surface can be paused while the document stays fully visible.
   */
  private hostSuspended = false;

  /** Drive the audio context alongside the game loop's suspended flag. */
  setSuspended(suspended: boolean) {
    if (this.hostSuspended === suspended) return;
