...
export class Kart implements IKart {
  ...
  /**
   * Remote-pose frame for network-owned karts: timers, terrain-following
   * wheels and the visual tail — but no physics, no progress, no watchdogs.
   * The network adapter has already written position/quaternion/velocity and
   * the presentation counters into this kart; everything here is downstream
   * of that pose.
   */
  stepRemote(ctx: Ctx, dt: number) {
    this.track = ctx.track;
    if (!Number.isFinite(dt)) return;
    dt = clamp(dt, 1 / 480, 1 / 20);
    ...
    // Wheels follow the terrain under the reported pose (cosmetic only).
    this.suspension.probeGround(ctx.track, this.position, this.quaternion, this.t);
    this.surface = this.suspension.contacts > 0 ? this.suspension.worstSurface : this.surface;
    // Basis vectors the drift-pose math reads; physics rebuilds them per
    // substep in `updateBasis`, which a remote kart never runs.
