...
Attendee projections preserve `responseStatus` (`needsAction`, `accepted`, `tentative`, `declined`, or `unknown`) and `isCurrentUser`. Viewer identity is matched against the connected Google account email from provider discovery; Google's attendee `self` flag instead identifies the calendar copy's owner and is not viewer identity on shared calendars. Missing RSVP data remains unknown. The UI uses the viewer's response independently of booking approval status, and a calendar owner's declined event does not block that calendar's availability.

Apply migration `0021_calendar_attendee_responses.sql` before deploying the gateway. It adds `calendar_sync_state.projection_version`; old projections stay at version 0 until a successful full snapshot replaces them. Queries and background sync rebuild those snapshots even when Google reports no changed events. Deploy the updated miniapp as well to render the response labels and event styles. Legacy local records remain readable and display unknown responses until refreshed.
