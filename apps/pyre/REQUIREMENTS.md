...
- [ ] Zephyr Cloud publication and host publication receipts (blocked: no SDK 0.4.1 publication API)
...
- VFS has write/provision operations but no public read/list/immutable-bit API in SDK 0.4.1; Pyre can write evidence snapshots and receipts but cannot independently browse or enforce host filesystem immutability.
...
- CKG, knowledge-plot, Zephyr Cloud publication, reminder scheduling, repository clone, notification, and access-request APIs are not present in SDK 0.4.1. Package workflow declarations also remain limited to one attempt without checkpointing or scheduling. Their executable controls must be omitted rather than simulated.
