...
- [ ] Zephyr Cloud publication and host publication receipts (blocked: no miniapp runtime publication API; SDK 0.9.0's authoring lifecycle publishes only through a configured build-time adapter)
...
- VFS has write/provision operations but no public read/list/immutable-bit API in SDK 0.9.0; Pyre can write evidence snapshots and receipts but cannot independently browse or enforce host filesystem immutability.
...
- SDK 0.9.0 exposes repository-scoped Code Knowledge reads, while its plot contract remains reserved and unavailable in the current host. Pyre has not yet added a host-resolved project binding for Code Knowledge. Runtime Zephyr publication, reminder scheduling, repository clone, and access-request APIs remain unavailable; immediate host-mediated OS notifications do not provide due-date scheduling. Unsupported executable controls are omitted rather than simulated.
