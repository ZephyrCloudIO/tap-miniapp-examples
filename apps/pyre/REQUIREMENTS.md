...
- [ ] Zephyr Cloud publication and host publication receipts (blocked: no miniapp runtime publication API; SDK 0.7.0's authoring lifecycle publishes only through a configured build-time adapter)
...
- VFS has write/provision operations but no public read/list/immutable-bit API in SDK 0.7.0; Pyre can write evidence snapshots and receipts but cannot independently browse or enforce host filesystem immutability.
