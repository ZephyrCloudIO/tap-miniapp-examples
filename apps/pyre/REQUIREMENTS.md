...
- [ ] Zephyr Cloud publication and host publication receipts (blocked: no SDK 0.5.2 publication API)
...
- VFS has write/provision operations but no public read/list/immutable-bit API in SDK 0.5.2; Pyre can write evidence snapshots and receipts but cannot independently browse or enforce host filesystem immutability.
...
- CKG, knowledge-plot, Zephyr Cloud publication, reminder scheduling, repository clone, and access-request APIs are not present in SDK 0.5.2. Immediate host-mediated OS notifications are available, but they do not provide Pyre's required due-date scheduling. Package workflow declarations also remain limited to one attempt without checkpointing or scheduling. Unsupported executable controls must be omitted rather than simulated.
