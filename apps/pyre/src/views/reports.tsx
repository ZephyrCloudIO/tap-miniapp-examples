...
import { buildReportMarkdown, canEdit, canReview, reportReadiness, timestamp, type Actor, type Investigation, type ReportRevision } from "../domain";
...
export function ReportsView({ investigation, actor, saving, authorize, onUpdate }: Props) {
  ...
  return <div className="view-stack">
    ...
    <Card className="publication-card"><CardHeader><CardTitle>Zephyr Cloud Publication</CardTitle><Badge variant="outline">Unavailable at runtime</Badge></CardHeader><CardContent><p>SDK 0.9.0 supports package publication through a configured build-time adapter, but miniapp runtime code cannot publish reports. Pyre therefore does not display a publish control or simulate a resulting URL. Approved report sources, HTML, digests, and revision history remain available for an authorized future publisher.</p></CardContent></Card>
