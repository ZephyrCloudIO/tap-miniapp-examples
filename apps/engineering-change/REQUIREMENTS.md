...
- The app-defined plot ledger (`plot.definition` + `sdk.plots`), repository-scoped CKG capability, Home attention projections, context actions with immutable artifact references, and idempotent task/integration actions with durable receipts are specified in the brief and tracked by [ZephyrCloudIO/ze-agency-tauri#7989](https://github.com/ZephyrCloudIO/ze-agency-tauri/issues/7989). SDK 0.4.6 cannot invoke them, so this package persists its ledger through `sdk.storage` with optimistic-concurrency revisions and omits the unavailable executable controls rather than simulating them.
...
- Repository issue creation from finding dispositions awaits the connected-integration actions in #7989; the disposition records the human's choice durably in the interim.
