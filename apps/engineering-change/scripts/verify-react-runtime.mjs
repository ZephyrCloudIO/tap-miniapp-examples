...
for (const dependency of ["react", "react-dom"]) {
  if (sharedNames.includes(dependency)) {
    throw new Error(
      `Engineering Change must bundle one private ${dependency} runtime; TAP initializes package Federation containers with an empty share scope.`,
    );
  }
}
...
if (!Array.isArray(javaScriptAssets) || javaScriptAssets.length === 0) {
  throw new Error("Engineering Change's desktop expose has no synchronous JavaScript asset.");
}
...
if (reactRuntimeDefinitions?.length !== 1) {
  throw new Error(
    `Engineering Change's desktop expose must contain exactly one React runtime; found ${reactRuntimeDefinitions?.length ?? 0}.`,
  );
}
if (reactDomRuntimeDefinitions?.length !== 1) {
  throw new Error(
    `Engineering Change's desktop expose must contain exactly one React DOM runtime; found ${reactDomRuntimeDefinitions?.length ?? 0}.`,
  );
}

console.log("verified one private React/React DOM runtime in Engineering Change's desktop expose");
